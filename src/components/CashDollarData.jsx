...
const CashDollarData = ({ data }) => {
  ...
  const groupedEstancia = groupByOTA(efectivoDolaresEstancia);
      ...
      <Card className="bg-dark">
        <Card.Body>
          <Card.Title className="text-center text-white">
            Ingreso en Efectivo en Dólares
          </Card.Title>
        </Card.Body>
      </Card>
        ...
        <Col>
          ...
          {Object.keys(groupedEstancia).map((ota) => (
            <Card className="mb-4" key={ota}>
              ...
              <Table striped bordered hover className="cash-dollar-data-table">
                <thead>
        ...
        <Col>
          ...
          <Card>
            <Table striped bordered hover>
              ...
              <tbody>
                {efectivoDolaresAmenidades.map((item, index) => (
                  <tr key={item._id}>
                    <td>{index + 1}</td>
                    <td>{new Date(item.fechaPago).toLocaleDateString()}</td>
                    <td>{item.nombre}</td>
